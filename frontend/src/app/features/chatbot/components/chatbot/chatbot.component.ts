...
export class ChatbotComponent {
  ...
  deletarConversa(id: string, event?: Event): void {
    event?.stopPropagation();

    this._conversas.update((lista) => {
      const proxima = lista.filter(c => c.id !== id);
      this.saveState(proxima);

      // Se a conversa ativa foi deletada, ativa a primeira da lista ou cria nova
      if (this.conversaAtivaId() === id) {
        if (proxima.length > 0) {
          this.conversaAtivaId.set(proxima[0].id);
        } else {
          this.novaConversa();
        }
      }
